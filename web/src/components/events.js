export default [
  {
    'title': 'All Day Event very long title',
    'bgColor': '#ff7f50',
    'allDay': true,
    'start': new Date(2020, 10, 1),
    'end': new Date(2020, 10, 1)
  },
  {
    'title': 'Blablabla',
    'bgColor': '#ff7f50',
    'allDay': true,
    'start': "2020-10-3",
    'end': "2020-10-4"
  },
  {
    'title': 'Long Event',
    'start': new Date(2020, 10, 7),
    'end': new Date(2020, 10, 10)
  },
  {
    'title': 'Some Event',
    'bgColor': '#9932cc',
    'start': new Date(2020, 10, 9, 0, 0, 0),
    'end': new Date(2020, 10, 9, 0, 0, 0)
  },
  {
    'title': 'Conference',
    'bgColor': '#e9967a',
    'start': new Date(2020, 10, 11),
    'end': new Date(2020, 10, 13),
    desc: 'Big conference for important people'
  },
  {
    'title': 'Meeting',
    'bgColor': '#8fbc8f',
    'start': new Date(2020, 10, 12, 10, 30, 0, 0),
    'end': new Date(2020, 10, 12, 12, 30, 0, 0),
    desc: 'Pre-meeting meeting, to prepare for the meeting'
  },
  {
    'title': 'Lunch',
    'bgColor': '#cd5c5c',
    'start':new Date(2020, 10, 12, 12, 0, 0, 0),
    'end': new Date(2020, 10, 12, 13, 0, 0, 0),
    desc: 'Power lunch'
  },
  {
    'title': 'Happy Hour',
    'start':new Date(2020, 10, 12, 12, 0, 0, 0),
    'end': new Date(2020, 10, 12, 13, 0, 0, 0),
    desc: 'Power lunch happy hour'
  },
  {
    'title': 'Meeting',
    'bgColor': '#da70d6',
    'start':new Date(2020, 10, 12,14, 0, 0, 0),
    'end': new Date(2020, 10, 12,15, 0, 0, 0)
  },
  {
    'title': 'Happy Hour',
    'bgColor': '#eee8aa',
    'start':new Date(2020, 10, 17, 17, 0, 0, 0),
    'end': new Date(2020, 10, 17, 17, 30, 0, 0),
    desc: 'Most important meal of the day'
  },
  {
    'title': 'Dinner',
    'bgColor': '#98fb98',
    'start':new Date(2020, 10, 15, 20, 0, 0, 0),
    'end': new Date(2020, 10, 15, 21, 0, 0, 0)
  },
  {
    'title': 'Birthday Party',
    'bgColor': '#afeeee',
    'start':new Date(2020, 10, 13, 7, 0, 0),
    'end': new Date(2020, 10, 13, 10, 30, 0)
  },
  {
    'title': 'Birthday Party 2',
    'bgColor': '#db7093',
    'start':new Date(2020, 10, 13, 7, 0, 0),
    'end': new Date(2020, 10, 13, 10, 30, 0)
  },
  {
    'title': 'Birthday Party 3',
    'bgColor': '#cd853f',
    'start':new Date(2020, 10, 13, 7, 0, 0),
    'end': new Date(2020, 10, 13, 10, 30, 0)
  },
  {
    'title': 'Late Night Event',
    'bgColor': '#b0e0e6',
    'start':new Date(2020, 10, 17, 19, 30, 0),
    'end': new Date(2020, 10, 18, 2, 0, 0)
  },
  {
    'title': 'Multi-day Event',
    'start':new Date(2020, 10, 20, 19, 30, 0),
    'end': new Date(2020, 10, 22, 2, 0, 0)
  }
]
